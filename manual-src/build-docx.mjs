...
const out = join(here, "..", "manuals", "交通服務水準分析系統_新手使用手冊_v2.20.1.docx");
...
const boxParagraphs = (block) => {
  ...
  const fill = block.variant === "warn" ? "FDF4E7" : "EEF6F7";
  const children = [];
  if (block.title)
    children.push(
      new Paragraph({
    ...
    new TableRow({
      children: cells.map((runs, index) =>
        cell(
          ...
          { width: columnWidths[index], fill: rowIndex % 2 ? "F8FBFC" : undefined },
        ),
      ),
    }),
  );
...
const doc = new Document({
  creator: "全日交通量及車種組成",
  title: "交通服務水準分析系統 新手使用手冊 v2.20.1",
        ...
        default: new Footer({
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: "v2.20.1 ｜ 2026-08-24 ｜ 使用前請先下載專案包備份　　第 ",
                  font: FONT,
                  size: 15,
                  color: MUTED,
                }),
