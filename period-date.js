// 交通服務水準分析系統｜調查日期 × 期別 一致性檢查
// 這一支沒有打包工具，所以用 IIFE 掛一個全域物件。
// 內容與「路口轉向」「全日交通量」的 lib/period-date.ts 是同一份程式，
// 只差在模組語法（那兩支是 export，這裡是 globalThis.PeriodDate），
// 且三支的 period-date 測試表逐字相同。要改請三支一起改。
(function (global) {
  "use strict";
  ...
  function parseSurveyPeriod(input) {
     ...
     * 看起來像個正常結果，反而比認不得更危險。
     */
    const toAd = (n) => (n >= 90 && n <= 200 ? n + 1911 : n);
    const month = (year, num) =>
  ...
  function periodOfDate(iso, kind) {
  ...
  function parseSurveyDateText(text) {
   ...
   * 兩層：(1) 有「日期：」標示的優先；(2) 都沒有才退而用第一個長得像日期的，
   * 並在訊息註明是推測，讓使用者知道要自己看一眼。
   */
  function findSurveyDate(cells) {
   ...
   */
  function checkPeriodAgainstDate(periodInput, found, fileLabel) {
   ...
   */
  function periodMismatchPrompt(checks) {
