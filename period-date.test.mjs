...
/*
 * period-date.js 是掛在 globalThis 的 IIFE（這一支沒有打包工具），
 * 所以這裡直接載入同一支檔案——測到的就是網站上跑的那一份。
 */
const require = createRequire(import.meta.url);
require(join(dirname(fileURLToPath(import.meta.url)), "period-date.js"));
const {
...
test("有標示的儲存格優先於位置較前、沒有標示的那一格", function () {
  ...
  assert.equal(found && found.iso, "2026-01-25");
  assert.equal(found && found.cell, "F3");
  assert.equal(found && found.labelled, true);
});

test("完全沒有標示時退而採用第一個像日期的，並標記為推測", function () {
  const found = findSurveyDate([{ text: "115年01月20日", sheet: "S", cell: "A1" }]);
  assert.equal(found && found.iso, "2026-01-20");
  assert.equal(found && found.labelled, false);
