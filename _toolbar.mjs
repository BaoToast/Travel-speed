 ...
 *   收合時那一排條件是 `hidden`，Playwright 的 selectOption／fill 會一直
 *   等它變成可見，最後逾時——**那不是程式壞了，是測試還停在「預設展開」
 *   的世界**。凡是要動主工具列欄位的端對端，載入之後先呼叫這一支。
 *
 * ⚠️ 不可以改成「直接把 hidden 拿掉」之類的繞道：那樣量到的就不是
 *   使用者按得到的狀態了。這裡走的是**跟使用者一樣的那顆鈕**。
 *
