// 交通服務水準分析系統｜圖旁說明的第 3 級「代表什麼狀況」與第 4 級「要怎麼處理」
// 這一支沒有打包工具，所以用 IIFE 掛一個全域物件。
// 內容與「路口轉向」lib/chart-levels.ts、「全日交通量」app/chart-levels.ts
// 是同一份程式，只差在模組語法（那兩支是 export，這裡是 globalThis.ChartLevels），
// 且三支共用同一份契約測試 chart-levels-contract.mjs。要改請三支一起改。
(function (global) {
  "use strict";
 ...
 */
  ...
  const LEVEL3_TITLE = "代表什麼狀況";
  const LEVEL4_TITLE = "要怎麼處理";
 ...
 */
  function heavyShareLevels(sharePercent) {
 ...
 */
  function peakConcentrationLevels(sharePercent, coveredHours) {
  if (!Number.isFinite(sharePercent) || sharePercent <= 0) return null;
 ...
 */
  function dayCompareLevels(ratio) {
 ...
 */
  function trendChangeLevels(changePercent, quarterCount) {
 ...
 * @param total          總筆數
 */
  function losLevels(worst, congestedStart, congestedCount, total) {
 ...
 */
  function turnShareLevels(turnLabel, sharePercent) {
  if (!Number.isFinite(sharePercent) || sharePercent < 0) return null;
  if (sharePercent >= 40)
    return {
      state:
 ...
 */
  function coverageLevels(coveredHours) {
  ...
  global.ChartLevels = {
    ...
    heavyShareLevels: heavyShareLevels,
    peakConcentrationLevels: peakConcentrationLevels,
    dayCompareLevels: dayCompareLevels,
    trendChangeLevels: trendChangeLevels,
    losLevels: losLevels,
    turnShareLevels: turnShareLevels,
